...
nest::aeif_cond_alpha_clopath_dynamics( double, const double y[], double f[], void* pnode )
{
  ...
  //TODO Where do these equation come from? they probably need some modification
...
void
nest::aeif_cond_alpha_clopath::update( Time const& origin, const long from, const long to )
{
...
void
nest::aeif_cond_alpha_clopath::handle( SpikeEvent& e )
{
...
void
nest::aeif_cond_alpha_clopath::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
nest::aeif_cond_alpha_clopath::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
