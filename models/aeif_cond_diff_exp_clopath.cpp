/*
 *  aeif_cond_diff_exp_clopath.cpp
 *
...
nest::RecordablesMap< nest::aeif_cond_diff_exp_clopath > nest::aeif_cond_diff_exp_clopath::recordablesMap_;
...
template <>
void
RecordablesMap< aeif_cond_diff_exp_clopath >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::V_M > );
  insert_( names::g_ex, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::G_EXC > );
  insert_( names::g_in, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::G_INH > );
  insert_( names::w, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::W > );
  insert_( names::V_th, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::V_TH > );
  insert_( names::u_bar_plus, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::U_BAR_PLUS > );
  insert_( names::u_bar_minus, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::U_BAR_MINUS > );
  insert_( names::u_bar_bar, &aeif_cond_diff_exp_clopath::get_y_elem_< aeif_cond_diff_exp_clopath::State_::U_BAR_BAR > );
}
}

extern "C" int
nest::aeif_cond_diff_exp_clopath_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef nest::aeif_cond_diff_exp_clopath::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aeif_cond_diff_exp_clopath& node = *( reinterpret_cast< nest::aeif_cond_diff_exp_clopath* >( pnode ) );
  ...
  f[ S::DG_EXC ] = -(dg_ex) / node.P_.tau_syn_ex_rise + (-(g_ex) / node.P_.tau_syn_ex_rise - dg_ex) / node.P_.tau_syn_ex_decay;
  // Synaptic Conductance (nS)
  f[ S::G_EXC ] = dg_ex ;

  f[ S::DG_INH ] = -(dg_in) / node.P_.tau_syn_in_rise + (-(g_in) / node.P_.tau_syn_in_rise - dg_in) / node.P_.tau_syn_in_decay;
  // Synaptic Conductance (nS)
  f[ S::G_INH ] = dg_in ;
...
nest::aeif_cond_diff_exp_clopath::Parameters_::Parameters_()
  : V_peak_( 20.0 )                             // mV
  , V_reset_( -60.0 )                           // mV 
  , t_ref_( 5.0 )                               // ms
  ...
  , a( 0.0 )                                    // nS 
  , b( 1000.0 )                                 // pA
  , tau_syn_ex_decay( 6.0 )                     // ms
  , tau_syn_ex_rise( 1.0 )                      // ms
  , tau_syn_in_decay( 2.0 )                     // ms
  , tau_syn_in_rise( 0.5 )                      // ms
  , I_e( 0.0 )                                  // pA ----- set to zero since it is probably not needed
  , gsl_error_tol( 1e-6 ) 
{
}

nest::aeif_cond_diff_exp_clopath::State_::State_( const Parameters_& p )
  : r_( 0 )
{
...
nest::aeif_cond_diff_exp_clopath::State_::State_( const State_& s )
  : r_( s.r_ )
{
  for ( size_t i = 0; i < STATE_VEC_SIZE; ++i )
  {
...
nest::aeif_cond_diff_exp_clopath::State_& nest::aeif_cond_diff_exp_clopath::State_::operator=( const State_& s )
{
  r_ = s.r_;
  for ( size_t i = 0; i < STATE_VEC_SIZE; ++i )
  {
...
void
nest::aeif_cond_diff_exp_clopath::Parameters_::get( DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::tau_syn_ex_decay, tau_syn_ex_decay );
  def< double >( d, names::tau_syn_ex_rise, tau_syn_ex_rise );
  def< double >( d, names::tau_syn_in_decay, tau_syn_in_decay );
  def< double >( d, names::tau_syn_in_rise, tau_syn_in_rise );
...
void
nest::aeif_cond_diff_exp_clopath::Parameters_::set( const DictionaryDatum& d, Node* node )
{
  ...
  updateValueParam< double >( d, names::tau_syn_ex_decay, tau_syn_ex_decay, node );
  updateValueParam< double >( d, names::tau_syn_ex_rise, tau_syn_ex_rise, node );
  updateValueParam< double >( d, names::tau_syn_in_decay, tau_syn_in_decay, node );
  updateValueParam< double >( d, names::tau_syn_in_rise, tau_syn_in_rise, node );
  ...
  if ( tau_syn_ex_decay <= 0 || tau_syn_ex_rise <= 0 || tau_syn_in_decay <= 0 || tau_syn_in_rise <= 0 || tau_w <= 0 || tau_V_th <= 0 || tau_u_bar_plus <= 0 || tau_u_bar_minus <= 0 || tau_u_bar_bar <= 0)
  {
...
void
nest::aeif_cond_diff_exp_clopath::State_::get( DictionaryDatum& d ) const
{
...
void
nest::aeif_cond_diff_exp_clopath::State_::set( const DictionaryDatum& d, const Parameters_&, Node* node )
{
...
nest::aeif_cond_diff_exp_clopath::Buffers_::Buffers_( aeif_cond_diff_exp_clopath& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_cond_diff_exp_clopath::Buffers_::Buffers_( const Buffers_&, aeif_cond_diff_exp_clopath& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aeif_cond_diff_exp_clopath::aeif_cond_diff_exp_clopath()
...
nest::aeif_cond_diff_exp_clopath::aeif_cond_diff_exp_clopath( const aeif_cond_diff_exp_clopath& n )
...
void
nest::aeif_cond_diff_exp_clopath::init_buffers_()
{
  ...
  B_.sys_.function = aeif_cond_diff_exp_clopath_dynamics;
...
void
nest::aeif_cond_diff_exp_clopath::calibrate()
{
  ...
  V_.g0_ex_ = (1.0 / P_.tau_syn_ex_rise - 1.0 / P_.tau_syn_ex_decay) / (P_.tau_syn_ex_decay - P_.tau_syn_ex_rise);
  V_.g0_in_ = (1.0 / P_.tau_syn_in_rise - 1.0 / P_.tau_syn_in_decay) / (P_.tau_syn_in_decay - P_.tau_syn_in_rise);
...
void
nest::aeif_cond_diff_exp_clopath::update( Time const& origin, const long from, const long to )
{
...
void
nest::aeif_cond_diff_exp_clopath::handle( SpikeEvent& e )
{
...
void
nest::aeif_cond_diff_exp_clopath::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
nest::aeif_cond_diff_exp_clopath::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
